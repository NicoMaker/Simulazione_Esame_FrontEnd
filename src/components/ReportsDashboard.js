...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="custom-tooltip">
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ color: p.color, fontSize: '0.82rem', marginTop: 2 }}>
          {p.name}: <strong>{p.name?.includes('€') || p.name === 'Fatturato' ? '€' : ''}{p.value?.toLocaleString('it-IT')}</strong>
        </div>
      ))}
    </div>
  );
...
export default function ReportsDashboard() {
  ...
  const avgRevenue = confirmed.length ? (totalRevenue / confirmed.length).toFixed(0) : 0;
  ...
  const topSpaces = spaces
    .map(s => ({
      ...
      fatturato: confirmed.filter(b => b.spaceId === s.id).reduce((acc, b) => acc + b.totalCost, 0),
    }))
  ...
  const tableData = [...bookings]
    .filter(b => {
      ...
      if (search && !b.customerName.toLowerCase().includes(search.toLowerCase()) && !b.spaceName.toLowerCase().includes(search.toLowerCase()) && !b.id.toLowerCase().includes(search.toLowerCase())) return false;
      return true;
    })
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <div className="page-subtitle">Analisi delle performance e andamento prenotazioni</div>
      </div>
      ...
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        <div className="stat-card blue">
          ...
          <div className="stat-value">€{totalRevenue.toLocaleString('it-IT')}</div>
        ...
        <div className="stat-card violet">
          <div className="stat-icon violet"><Euro size={20} /></div>
      ...
      <div className="charts-grid" style={{ marginBottom: 20 }}>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ paddingTop: 8 }}>
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie data={byType} cx="50%" cy="50%" outerRadius={90} dataKey="value" labelLine={false} label={PieLabel}>
                  {byType.map((_, i) => <Cell key={i} fill={COLORS_PIE[i]} />)}
                </Pie>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ paddingTop: 8 }}>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={topSpaces} margin={{ top: 0, right: 10, bottom: 0, left: -20 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="name" tick={{ fontSize: 10 }} />
                <YAxis tick={{ fontSize: 10 }} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="prenotazioni" name="Prenotazioni" fill="var(--accent)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card-header">
          <div className="card-title">📈 Andamento Fatturato (ultimi 20 giorni)</div>
          <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
            Tasso di occupazione medio: <strong style={{ color: 'var(--accent)' }}>{occupancyRate}%</strong>
          </div>
        </div>
        <div className="card-body" style={{ paddingTop: 8 }}>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={last20} margin={{ top: 0, right: 20, bottom: 0, left: -10 }}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Legend formatter={(v) => <span style={{ color: 'var(--text-secondary)', fontSize: 12 }}>{v}</span>} />
              <Line yAxisId="left" type="monotone" dataKey="Fatturato" stroke="var(--emerald)" strokeWidth={2.5} dot={false} activeDot={{ r: 5 }} />
              <Line yAxisId="right" type="monotone" dataKey="Prenotazioni" stroke="var(--accent)" strokeWidth={2} dot={false} strokeDasharray="4 2" activeDot={{ r: 5 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <div className="card-body" style={{ paddingTop: 8 }}>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={weeklyRevenue} margin={{ top: 0, right: 20, bottom: 0, left: -10 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="settimana" tick={{ fontSize: 11 }} />
              <YAxis tick={{ fontSize: 10 }} />
              <Tooltip content={<CustomTooltip />} />
              <Legend formatter={(v) => <span style={{ color: 'var(--text-secondary)', fontSize: 12 }}>{v}</span>} />
              <Bar dataKey="Fatturato" fill="var(--accent)" radius={[4, 4, 0, 0]} />
              <Bar dataKey="Prenotazioni" fill="var(--violet)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8rem', color: 'var(--text-secondary)', cursor: 'pointer' }}>
              <input type="checkbox" checked={cancelledFilter} onChange={e => setCancelledFilter(e.target.checked)} style={{ accentColor: 'var(--accent)' }} />
              Mostra annullate
            </label>
          </div>
        </div>
        <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <div className="filter-bar" style={{ marginBottom: 0 }}>
            {[{ id: 'all', label: 'Tutti' }, { id: 'desk', label: '💻 Scrivanie' }, { id: 'meeting', label: '🤝 Riunioni' }, { id: 'office', label: '🏠 Uffici' }].map(f => (
              <button key={f.id} className={`filter-chip ${typeFilter === f.id ? 'active' : ''}`} onClick={() => setTypeFilter(f.id)}>{f.label}</button>
            ))}
          </div>
          <div className="search-wrap" style={{ marginLeft: 'auto' }}>
            <Search className="search-icon" />
            <input className="search-input" placeholder="Cerca..." value={search} onChange={e => setSearch(e.target.value)} />
            {search && <button onClick={() => setSearch('')} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)', cursor: 'pointer', background: 'none', border: 'none' }}><X size={13} /></button>}
          </div>
        </div>
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {tableData.length === 0 ? (
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Nessuna prenotazione trovata</td></tr>
              ) : tableData.map(b => {
                ...
                return (
                  <tr key={b.id} style={{ opacity: b.status === 'cancelled' ? 0.5 : 1 }}>
                    <td><span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.82rem', color: 'var(--accent)' }}>{b.id}</span></td>
                    <td>{b.customerName}</td>
                    <td>{b.spaceName}</td>
                    <td><span className={`badge badge-${b.spaceType}`}>{SPACE_TYPE_LABELS[b.spaceType]}</span></td>
                    <td>{date}</td>
                    <td>{b.startTime} – {b.endTime}</td>
                    <td>{b.hours}h</td>
                    <td style={{ color: 'var(--emerald)', fontWeight: 700, fontFamily: 'var(--font-display)' }}>€{b.totalCost}</td>
                    <td><span className={`badge badge-${b.status}`}>{b.status === 'confirmed' ? 'Confermata' : 'Annullata'}</span></td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div style={{ padding: '12px 18px', borderTop: '1px solid var(--border)', fontSize: '0.78rem', color: 'var(--text-muted)' }}>
