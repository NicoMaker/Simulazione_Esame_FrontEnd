...
import { CheckCircle, Calendar, Clock, User, MonitorPlay, Users, Briefcase, Building2, Check, ArrowLeft, ArrowRight } from 'lucide-react';
        ...
        return (
          <React.Fragment key={i}>
            <div className={`step ${state}`}>
              <div className="step-dot">{i < current ? <Check size={14} /> : i + 1}</div>
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.05rem', marginBottom: 4 }}>Seleziona lo spazio</div>
        <div style={{ fontSize: '0.82rem', color: 'var(--text-secondary)' }}>Sono mostrati solo gli spazi attualmente disponibili</div>
      </div>
      <div className="filter-bar">
        {[{ id: 'all', label: 'Tutti' }, { id: 'desk', label: '💻 Scrivanie' }, { id: 'meeting', label: '🤝 Sale Riunioni' }, { id: 'office', label: '🏠 Uffici' }].map(f => (
              ...
              <div key={space.id} className={`space-select-item ${isSelected ? 'selected' : ''}`} onClick={() => onSelect(space)}>
                ...
                <div className="space-select-info">
                  ...
                  <div className="space-select-sub">
                    {SPACE_TYPE_LABELS[space.type]} · Piano {space.floor} · {space.capacity} pers. · {(space.amenities || []).slice(0, 3).join(', ')}
                  </div>
                </div>
                <div className="space-select-rate">€{space.hourlyRate}<span style={{ fontSize: '0.7rem', fontFamily: 'var(--font-body)', fontWeight: 400, color: 'var(--text-muted)' }}>/h</span></div>
                {isSelected && <div style={{ width: 24, height: 24, borderRadius: '50%', background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Check size={13} color="white" /></div>}
              </div>
            );
          })}
        </div>
  ...
  return (
    <div>
      <div style={{ marginBottom: 18 }}>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.05rem', marginBottom: 4 }}>Dati Prenotazione</div>
      ...
      {success && (
        <div className="success-banner">
          ...
          <div>
            ...
            <div style={{ fontSize: '0.78rem', color: 'var(--emerald)', opacity: 0.8, marginTop: 2 }}>{success.customerName} · {success.spaceName} · €{success.totalCost}</div>
          </div>
              ...
              return (
                <div key={b.id} style={{
                  ...
                  <div style={{
                    ...
                    fontSize: '0.75rem', fontWeight: 800, color: TYPE_COLORS[b.spaceType],
