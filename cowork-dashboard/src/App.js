...
export default function App() {
  ...
  const addBooking = (booking) => {
    const newBooking = { ...booking, id: `BK${String(bookings.length + 1).padStart(3, '0')}`, status: 'confirmed', createdAt: new Date().toISOString() };
  ...
  const contextValue = {
    ...
    spaces, addSpace, updateSpaceStatus, deleteSpace,
    bookings, addBooking, cancelBooking,
    sidebarOpen, setSidebarOpen,
  };

  const pages = { home: DashboardHome, spaces: SpacesManager, booking: BookingSystem, reports: ReportsDashboard };
